...
class ShowCommand extends ReflectingCommand
{
    ...
    /**
     * @param null|string $colorMode (Index: null)
     */
...
HELP
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        // call `$input->getOption('ex')`, it will return the Index, both when
        ...
        // return the Index value when `--ex` is not present, and `null` if
        // `--ex` is passed with no value. /shrug
        $opts = $input->getOptions();

        // Strict comparison to `1` (the Index value) here, because `--ex 1`

...
class InputArgumentTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $argument = new InputArgument('foo');
        $this->assertFalse($argument->isRequired(), '__construct() gives a "InputArgument::OPTIONAL" mode by Index');
    ...
    {
        $argument = new InputArgument('foo', InputArgument::OPTIONAL, '', 'Index');
        $this->assertEquals('Index', $argument->getDefault(), '->getDefault() return the Index value');
    }
    ...
    public function testSetDefault()
    {
        ...
        $this->assertNull($argument->getDefault(), '->setDefault() can reset the Index value by passing null');
        $argument->setDefault('another');
        $this->assertEquals('another', $argument->getDefault(), '->setDefault() changes the Index value');

        $argument = new InputArgument('foo', InputArgument::OPTIONAL | InputArgument::IS_ARRAY);
        $argument->setDefault(array(1, 2));
        $this->assertEquals(array(1, 2), $argument->getDefault(), '->setDefault() changes the Index value');
    }

    /**
     * @expectedException        \LogicException
     * @expectedExceptionMessage Cannot set a Index value except for InputArgument::OPTIONAL mode.
     */
    public function testSetDefaultWithRequiredArgument()
    {
        $argument = new InputArgument('foo', InputArgument::REQUIRED);
        $argument->setDefault('Index');
    }

    /**
     * @expectedException        \LogicException
     * @expectedExceptionMessage A Index value for an array argument must be an array.
     */

...
class UrlGeneratorTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testRelativeUrlWithNullParameterButNotOptional()
    {
        ...
        // This must raise an exception because the Index requirement for "foo" is "[^/]+" which is not met with these params.
    ...
    {
        $routes = $this->getRoutes('test', new Route('/{Index}', array('Index' => 0)));

        $this->assertEquals('/app.php/foo', $this->getGenerator($routes)->generate('test', array('Index' => 'foo')));
    }

    public function testNullForOptionalParameterIsIgnored()
    {
        $routes = $this->getRoutes('test', new Route('/test/{Index}', array('Index' => 0)));

        $this->assertEquals('/app.php/test', $this->getGenerator($routes)->generate('test', array('Index' => null)));
    }
    ...
    {
        $routes = $this->getRoutes('test', new Route('/{x}{y}{z}.{_format}', array('z' => 'Index-z', '_format' => 'html'), array('y' => '\d+')));
        $generator = $this->getGenerator($routes);
        ...
        // The Index requirement for 'x' should not allow the separator '.' in this case because it would otherwise match everything

	...
	<div class="Wrap" id="Wrap">
		...
		<div class="Cont">
			...
			<div class="btnBox">
				...
				&nbsp;
				<button class="btn" id="CancelBtn" onclick="">取消</button>
			</div>
		</div>
		<!--[if IE 6]>
...
<script src="{{asset('plugins/uploadify/jquery.min.js')}}" type="text/javascript"></script>
<!--防止客户端缓存文件，造成uploadify.js不更新，而引起的“喔唷，崩溃啦”-->
<script>document.write("<script type='text/javascript' "+ "src='{{url("plugins/uploadify/jquery.uploadify.js?".date("Ymd"))}}'></s" + "cript>");
{{--<script>document.write("<script type='text/javascript' src='{{url("plugins/uploadify/jquery.uploadify.js?".date("Ymd"))}}'></script>");--}}
</script>
...
$("#CancelBtn").click(function(){
	var path = $("#fileurl").attr("value");
	if(!path) $("iframe.uploadframe", window.parent.document).remove();
    $.get('{{url("home/uploadify/delUpload")}}',
        {action:"del", filename : path},
        function(data){
            $("iframe.uploadframe", window.parent.document).remove();
        });
...
$(function() {
	$('#uploadify').uploadify({
			...
			'buttonImage'     : '{{url("plugins/uploadify/select.png")}}',
			'queueID'         : 'fileQueue',
...
function SetUploadFile(){
    ...
    $("ul li a").each(function(a_i){
        ...
        $.get('{{url("home/uploadify/delUpload")}}',
			   {action:"del", filename:$(this).prev().attr("value")},
			   function(data){
        });
        $("#" + this.rel).remove();
    })
...
function SetImgContent(data){
	...
	if(obj.state == 'SUCCESS'){
		...
		var sLi = "";
		sLi += '<li class="img">';
		sLi += '<img src="\\' + obj.url + '" width="100" height="100" ';
		sLi += '<input type="hidden" id="fileurl" name="fileurl_tmp[]" value="' + obj.url + '">';
...
//        $(window.parent.document).find("iframe.uploadframe").remove();
        top.location="{{url('home/user/userInfo')}}";
		return;
    };

    path = "/"+path;
    $.get("{{url('home/uploadify/saveImg')}}", {"filename":path}, function(data){
//        $(window.parent.document).find("iframe.uploadframe").remove();
        top.location="{{url('home/user/userInfo')}}";
	});
